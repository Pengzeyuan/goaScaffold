...
import (
	...
	goaHttpWr "goa.design/goa/v3/http/middleware"
	goaMl "goa.design/goa/v3/middleware"
)
...
// timing information.
func Log(l goaMl.Logger) func(h http.Handler) http.Handler {
	return func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			reqID := r.Context().Value(goaMl.RequestIDKey)
			if reqID == nil {
				reqID = shortID()
			}
			started := time.Now()
			...
			rw := goaHttpWr.CaptureResponse(w)
			h.ServeHTTP(rw, r)
