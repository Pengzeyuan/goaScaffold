// Code generated by goa v3.2.3, DO NOT EDIT.
//
// User client HTTP transport
//
// Command:
// $ goa gen boot/design

package client

import (
	"context"
	"net/http"

	goahttp "goa.design/goa/v3/http"
	goa "goa.design/goa/v3/pkg"
)

// Client lists the User service endpoint HTTP clients.
type Client struct {
	// LoginByUsername Doer is the HTTP client used to make requests to the
	// LoginByUsername endpoint.
	LoginByUsernameDoer goahttp.Doer

	// LoginBySmsCode Doer is the HTTP client used to make requests to the
	// LoginBySmsCode endpoint.
	LoginBySmsCodeDoer goahttp.Doer

	// UpdatePassword Doer is the HTTP client used to make requests to the
	// UpdatePassword endpoint.
	UpdatePasswordDoer goahttp.Doer

	// GetCaptchaImage Doer is the HTTP client used to make requests to the
	// GetCaptchaImage endpoint.
	GetCaptchaImageDoer goahttp.Doer

	// SendSmsCode Doer is the HTTP client used to make requests to the SendSmsCode
	// endpoint.
	SendSmsCodeDoer goahttp.Doer

	// RestoreResponseBody controls whether the response bodies are reset after
	// decoding so they can be read again.
	RestoreResponseBody bool

	scheme  string
	host    string
	encoder func(*http.Request) goahttp.Encoder
	decoder func(*http.Response) goahttp.Decoder
}

// NewClient instantiates HTTP clients for all the User service servers.
func NewClient(
	scheme string,
	host string,
	doer goahttp.Doer,
	enc func(*http.Request) goahttp.Encoder,
	dec func(*http.Response) goahttp.Decoder,
	restoreBody bool,
) *Client {
	return &Client{
		LoginByUsernameDoer: doer,
		LoginBySmsCodeDoer:  doer,
		UpdatePasswordDoer:  doer,
		GetCaptchaImageDoer: doer,
		SendSmsCodeDoer:     doer,
		RestoreResponseBody: restoreBody,
		scheme:              scheme,
		host:                host,
		decoder:             dec,
		encoder:             enc,
	}
}

// LoginByUsername returns an endpoint that makes HTTP requests to the User
// service LoginByUsername server.
func (c *Client) LoginByUsername() goa.Endpoint {
	var (
		encodeRequest  = EncodeLoginByUsernameRequest(c.encoder)
		decodeResponse = DecodeLoginByUsernameResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v interface{}) (interface{}, error) {
		req, err := c.BuildLoginByUsernameRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.LoginByUsernameDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("User", "LoginByUsername", err)
		}
		return decodeResponse(resp)
	}
}

// LoginBySmsCode returns an endpoint that makes HTTP requests to the User
// service LoginBySmsCode server.
func (c *Client) LoginBySmsCode() goa.Endpoint {
	var (
		encodeRequest  = EncodeLoginBySmsCodeRequest(c.encoder)
		decodeResponse = DecodeLoginBySmsCodeResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v interface{}) (interface{}, error) {
		req, err := c.BuildLoginBySmsCodeRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.LoginBySmsCodeDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("User", "LoginBySmsCode", err)
		}
		return decodeResponse(resp)
	}
}

// UpdatePassword returns an endpoint that makes HTTP requests to the User
// service UpdatePassword server.
func (c *Client) UpdatePassword() goa.Endpoint {
	var (
		encodeRequest  = EncodeUpdatePasswordRequest(c.encoder)
		decodeResponse = DecodeUpdatePasswordResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v interface{}) (interface{}, error) {
		req, err := c.BuildUpdatePasswordRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.UpdatePasswordDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("User", "UpdatePassword", err)
		}
		return decodeResponse(resp)
	}
}

// GetCaptchaImage returns an endpoint that makes HTTP requests to the User
// service GetCaptchaImage server.
func (c *Client) GetCaptchaImage() goa.Endpoint {
	var (
		decodeResponse = DecodeGetCaptchaImageResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v interface{}) (interface{}, error) {
		req, err := c.BuildGetCaptchaImageRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.GetCaptchaImageDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("User", "GetCaptchaImage", err)
		}
		return decodeResponse(resp)
	}
}

// SendSmsCode returns an endpoint that makes HTTP requests to the User service
// SendSmsCode server.
func (c *Client) SendSmsCode() goa.Endpoint {
	var (
		encodeRequest  = EncodeSendSmsCodeRequest(c.encoder)
		decodeResponse = DecodeSendSmsCodeResponse(c.decoder, c.RestoreResponseBody)
	)
	return func(ctx context.Context, v interface{}) (interface{}, error) {
		req, err := c.BuildSendSmsCodeRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		err = encodeRequest(req, v)
		if err != nil {
			return nil, err
		}
		resp, err := c.SendSmsCodeDoer.Do(req)
		if err != nil {
			return nil, goahttp.ErrRequestError("User", "SendSmsCode", err)
		}
		return decodeResponse(resp)
	}
}
