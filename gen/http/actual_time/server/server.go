...
// Server lists the ActualTime service endpoint HTTP handlers.
type Server struct {
	Mounts                    []*MountPoint
...
// New instantiates HTTP handlers for all the ActualTime service endpoints
...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			{"GetActualTimeData", "GET", "/api/actual_time/get_actual_time_data"},
			{"ReceiveThirdPartyPushData", "POST", "/api/actual_time/receive_third_party_push_data"},
		},
...
func (s *Server) Service() string { return "ActualTime" }
...
// Mount configures the mux to serve the ActualTime endpoints.
...
// MountGetActualTimeDataHandler configures the mux to serve the "ActualTime"
...
func MountGetActualTimeDataHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/api/actual_time/get_actual_time_data", f)
}

// NewGetActualTimeDataHandler creates a HTTP handler which loads the HTTP
// request and calls the "ActualTime" service "GetActualTimeData" endpoint.
func NewGetActualTimeDataHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ctx = context.WithValue(ctx, goa.ServiceKey, "ActualTime")
		var err error
...
// "ActualTime" service "ReceiveThirdPartyPushData" endpoint.
func MountReceiveThirdPartyPushDataHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/api/actual_time/receive_third_party_push_data", f)
}

// NewReceiveThirdPartyPushDataHandler creates a HTTP handler which loads the
// HTTP request and calls the "ActualTime" service "ReceiveThirdPartyPushData"
// endpoint.
